...
Instagram's reads "oficial do clube". Coverage reached all twenty clubs on
2026-09-16 and may shrink again — a community can be abandoned, and then the
entry goes rather than the page keeping a link to a dead room. A club with no
entry renders no link, because `r/<club name>` is exactly the address somebody
would derive: five of those turned out to name a Mexican club, a Portuguese one,
the Civil Air Patrol, a state and a city. The bar is **two independent sources naming the same sub** —
...
**Perfil no Sofascore** rule. From a signed-in browser the same request does
answer honestly (a real sub, an unregistered name and a lookalike are three
different answers, told apart by the payload's `kind` and never by its status),
which is how a re-check is done by hand; it still cannot run in CI.
_Avoid_: keying on **tla** (Corinthians and Coritiba share `COR`, so one club's
...
**Marca da camisa**:
The shirt number a second time on the **Cartão do jogador**, as a small solid
badge pinned to the corner of the player's photograph — the card's one piece
of decoration. It is not a duplicate of the *Camisa* **Ficha** competing with
it: the Ficha is read, the badge is recognised, which is what lets it carry a
number printed again below and stay `aria-hidden`. Absent until the enrichment
answers, since the competition's team payload carries no shirt number for
anybody, and absent even then for a player with no recorded photograph — with
no picture to pin it to, the badge has nothing to be recognised from and the
Ficha alone carries the number.
_Avoid_: "marca d'água" (the file and the specs call every one of these a
*marca*, and this is the shirt's — it stopped being a watermark when it moved
onto the photo), reading it as a value, and rendering it with no photograph
beside it.
