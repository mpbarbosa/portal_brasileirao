...
runs it: the seed and the counters are read by real node against the checkout,
while `curl`, `npx` and `npm` are stubbed so the caRtola probe and the syncs are
observable. **The order is the property it exists for** — the scouts sync divides
caRtola's counters by the seed's match counts, so a lagging seed inflates every
rate — and it was confirmed by mutation rather than trusted: swapping the two
syncs fails 5 of its 19 assertions, dropping the root guard fails 11, making the
seed sync non-fatal fails 2, and letting an unknown option mean `--check` fails 1.
