...
- `goals-core.ts` — the **gols** under the Partida scoreline: who scored, and for
  ...
  **`Goal.clubCode` is the club the goal *counts for*, not the scorer's club**,
  and the whole design turns on that distinction. An own goal is the one case
  where they differ, and getting it backwards puts a goal on the wrong side of a
  scoreboard while looking entirely plausible. So the vocabulary is closed
  rather than open: `isKnownGoalResult` reports whether a `resultado` is one
  that has actually been *observed* (`NR`, `PN`), and the sync **refuses** a code
  it does not know. That is deliberately the opposite of `refereeRoleLabel` one
  module over, where an unmapped value renders verbatim — there the cost of
  being wrong is an English word on the page, here it is a wrong scoreline. Note
  `GoalKind` declares `own` with **no code mapped to it**: no own goal has been
  seen in a payload yet, so CBF's code for one is unmeasured and is not guessed.
...
production never calls it, exactly as for `broadcasts.ts`. Every entry has been
