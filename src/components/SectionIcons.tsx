...
import { GLYPH_STROKE } from "@/src/components/glyph";
...
/*
 * The icons below are not sections, and they live here anyway.
 *
 * What they share with the five above is what kind of mark they are: a
 * **standalone icon**, sized by the control or card that holds it, rather than
 * a glyph set inline in a line of text the way `ClubLinks`' marks are. That is
 * the rule this file follows, and `NAV_ITEMS` reaching only for the first five
 * is a fact about that list rather than about this module. The stroke they are
 * drawn with is `GLYPH_STROKE` in `glyph.ts`, which every hand-drawn glyph in
 * the app shares — so an icon here and a mark defined beside its call site
 * cannot drift apart, wherever the second one lives.
 */
 ...
 *
 * Here rather than beside the toggle for the reason `SunIcon` is: it is a
 * standalone icon, which is what this file draws. Neither of these is a section
 * either.
 *
 ...
 * `MoonIcon` do: each is a standalone icon, sized by the card that holds it. And they are
