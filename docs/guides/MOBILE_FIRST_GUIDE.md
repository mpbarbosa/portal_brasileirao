...
enhancement is enforced by the tool. **The base is the phone.**
...
79 dp — so the indicator degrades to 56 dp under `min-[360px]:` and only there
...
ten fixture links at 24 px on Jogos and roughly 950 player-name buttons on
Jogadores are links *inside content*, not targets beside it. Raising them means
re-laying-out three pages, and a spec asserting "every anchor is 48 dp" would
fail on all of them until somebody deleted the spec.
...
- **`STICKY_CLUB`'s `left-*` must equal `STICKY_POSITION`'s `w-*`.** Today
  `w-14` and `left-14` (`src/components/StandingsTable.tsx:271` and `:295`).
  Widen the position column alone and the two frozen columns overlap or gap —
  and only while scrolled, because ordinary table layout puts them adjacent
  either way.
...
chrome above `sm` (73 → 105); below `sm` the bottom navigation bar is unchanged
and the header stays 73.
...
1. **Measure at 320, 360 and 375 dp.** Nine specs already call
   `setViewportSize`; add to them rather than reasoning about the arithmetic.
...
5. **The touch target and the visible box are separate decisions**, and a
   `min-height` beats a `height` whatever the order.
6. **Name the set a target-size spec measures**; do not infer it from a selector.
7. **A non-uniform scale is for filled rects only.**
8. **Add a breakpoint when the layout breaks**, not per device.

## Current reality

- **The nav bar is at MD3's maximum and has no slack left.** `/trafego`,
  `/conta`, `/entrar` and `/privacidade` are all reached without a nav entry.
- **`tests/e2e/tabs-and-targets.spec.ts` carries a written exclusion list** for
  the inline links, and the reasoning lives in `docs/md3-completion-plan.md`
  under M9 so it reads as a decision rather than an oversight.
- **Nine specs set a viewport**; the rest run at the default. A layout regression
  outside those nine is not caught.
...
screen? Do the arithmetic, signed in.
...
**Scale test.** Does this drawing contain anything that is not a filled rect?
...
- A webfont, an icon package, or a hotlinked image.
...
- [ ] Touch target and visible box decided separately; no `min-h`/`h` collision.
- [ ] Drawing scales uniformly unless every mark is a filled rect.
