...
test.describe("API", () => {
  ...
  /**
   * The artilharia, asserted as a RANKING rather than as a list.
   *
   * Two things were measured against the seed before being written down, and
   * each changed a line. At least one nullable field is **genuinely null**, so
   * the null-or-number check is not a vacuous one — `assists`, `penalties` and
   * `playedMatches` are all absent for somebody. And **club codes are not
   * unique** (13 distinct across 20 rows when this landed): two scorers share a
   * club, so uniqueness — the obvious thing to copy from the standings spec
   * three tests up — would be a *wrong* assertion rather than a missing one.
   *
   * No count, and no name: the list is regenerated by `sync-seed-data`, so its
   * length and who leads it are both facts about when the snapshot was taken.
   */
  test("/api/scorers ranks scorers, and every club is one in the division", async ({ request }) => {
    ...
    rows.forEach((row, i) => {
      // A rank, so 1-based and dense: a gap would mean a row was dropped after
      // the positions were assigned.
      expect(row.position).toBe(i + 1);
      // Sorted by goals, which is what makes it an artilharia rather than a set.
      if (i > 0) expect(rows[i - 1].goals).toBeGreaterThanOrEqual(row.goals);

      expect(row.playerId.length).toBeGreaterThan(0);
      expect(row.playerName.trim().length).toBeGreaterThan(0);
      // Carries the whole club, so the page needs no second lookup — and it has
      // to be a club in this division, which is what catches a scorer stranded
      // by a transfer or a code from another source.
      expect(codes).toContain(row.club.code);

      // Goals are the one figure that must be a number: a scorer with no goals
      // is not in an artilharia.
      expect(typeof row.goals).toBe("number");

      // The other three are reported as an explicit **null** where upstream said
      // nothing, and the key has to be THERE. `JSON.stringify` drops an
      // `undefined` value, so a field that stopped being set arrives as a
      // missing key rather than as a null — which a `typeof` check cannot see,
      // and which a client reading `row.assists ?? 0` would render as a real
      // zero. Note what this does NOT claim: a 0 written where a null belonged
      // is indistinguishable from a genuine 0 at this layer, so nothing here
      // guards that — `tests/football-data-core.test.ts` owns the mapping.
      for (const key of ["assists", "penalties", "playedMatches"] as const) {
        expect(key in row, `${key} missing from the payload, not null`).toBe(true);
        if (row[key] !== null) expect(typeof row[key]).toBe("number");
      }
    });
  });

  /**
   * The traffic dashboard, asserted through its own COUPLINGS.
   *
   * **The bound is worth reading before extending this.** The suite's cwd has no
   * `traffic-reports/` directory, so the route answers the documented empty
   * shape — which is production's shape on any host where the timer has not run,
   * and is the branch a reader of `/trafego` meets first. The populated branches
   * below are therefore guarded and vacuous *here*; they are covered by
   * `tests/traffic-report-core.test.ts`, which drives `buildTrafficDashboard`
   * against the output of a real run of the shell script. What this adds is the
   * half no unit test can reach: that the route exists, is wired to the parser,
   * and answers an envelope.
   *
   * It deliberately does **not** pin `source: "fallback"`. That is a fact about
   * whether a directory exists next to the server, so a spec asserting it would
   * go red the day somebody adds a fixture — and the couplings below are true
   * either way.
   */
  test("/api/traffic-dashboard answers an envelope whose parts agree", async ({ request }) => {
    const response = await request.get("/api/traffic-dashboard");
    expect(response.status()).toBe(200);
    ...
    const data = body.data as {
      ...
      timeline: { ratePerMin: number | null }[];
      latest: unknown;
    };

    expect(Number.isInteger(data.snapshotCount)).toBe(true);
    expect(data.snapshotCount).toBeGreaterThanOrEqual(0);
    // One point per snapshot parsed. The count and the timeline are two readings
    // of one walk, so they cannot disagree without one of them being wrong.
    expect(data.timeline).toHaveLength(data.snapshotCount);
    // Documented on the type: zero snapshots is the fallback shape, with a null
    // latest. Stated as an iff, because either half alone passes vacuously.
    expect(data.latest === null).toBe(data.snapshotCount === 0);

    if (data.snapshotCount < 2) {
      // No interval to divide by — "not measurable" rather than a rate of zero.
      expect(data.windowRatePerMin).toBeNull();
    }

    if (data.timeline.length > 0) {
      // The first snapshot has nothing to difference against, so its rate is
      // null and NOT zero: an hour that served nobody and an hour nobody can
      // measure are different readings, and the chart draws them differently.
      expect(data.timeline[0].ratePerMin).toBeNull();
    }

    for (const point of data.timeline) {
      // Clamped at zero: logrotate drops the oldest lines, so a cumulative
      // total legitimately falls across a rotation and the honest answer is no
      // measurable rate rather than a negative one.
      if (point.ratePerMin !== null) expect(point.ratePerMin).toBeGreaterThanOrEqual(0);
    }
  });
