...
/**
 * Open a finished fixture carrying one broadcast line: three broadcasters with a
 * mark, and Record, which has none.
 *
 * **Produced, not pinned.** These tests opened 554972, whose curated line
 * happened to be exactly that — and `broadcasts.ts` is regenerated by
 * `sync-broadcasts`, which is free to rewrite it. The names are what the
 * assertions below are about, so they are written here where those assertions
 * can be read against them.
 */
const openBroadcastLine = async (page: Page) => {
  const body = await readMatches(page);
  const match = finishedFixture(body);
  match.broadcasters = ["Premiere", "Record", "YouTube", "Cazé TV"];
...
test.describe("Onde assistir", () => {
  ...
  test("the highlights name every channel in text, with no mark to rest on", async ({ page }) => {
    // ge tv plays in the page and CazéTV refuses to, so one channel is named
    // beside the frame and the other as a link. Produced rather than read off
    // 554975, whose curated line is `highlights.ts`' to rewrite.
    const body = await readMatches(page);
    const match = finishedFixture(body);
    match.highlights = [
